...
export const generateSubjectPlan = (state: StudyState, subjectId: string): PlannerResult => {
  ...
  availableLectures.forEach((lecture) => {
    let assignedDay: StudyDay | null = null;
    for (const day of planningDays) {
      const assignments = dayAssignments.get(day.date)!;
      const minutes = dayMinutes.get(day.date)!;
      const exceedsLectures = assignments.length + 1 > maxLectures;
      const exceedsMinutes = minutes + lecture.estimatedMinutes > maxMinutes;
      if (!exceedsLectures && !exceedsMinutes) {
        assignedDay = day;
        break;
      }
    }
    ...
    if (!assignedDay) {
      assignedDay = planningDays.reduce((best, day) => {
        const bestAssignments = dayAssignments.get(best.date) ?? [];
        const currentAssignments = dayAssignments.get(day.date) ?? [];
        return currentAssignments.length < bestAssignments.length ? day : best;
      }, planningDays[0]);
      overloaded = true;
    }
    ...
    const newMinutes = (dayMinutes.get(assignedDay.date) ?? 0) + lecture.estimatedMinutes;
