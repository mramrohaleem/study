...
interface StudyActions {
  ...
  setDayMeta(date: string, meta: { mood?: number; energy?: number }): void;
...
export const StudyProvider: React.FC<StudySectionProps & { children: React.ReactNode }> = ({
  ...
  const actions = useMemo<StudyActions>(() => ({
    ...
    setDayMeta: (date, meta) => {
      updateState((prev) => {
        const nextMeta = [...prev.dayMeta];
        const existingIndex = nextMeta.findIndex((item) => item.date === date);
        if (existingIndex >= 0) {
          nextMeta[existingIndex] = { ...nextMeta[existingIndex], ...meta, date };
        } else {
          nextMeta.push({ date, ...meta });
        }
        return { ...prev, dayMeta: nextMeta };
      });
    },
